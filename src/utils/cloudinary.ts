...
export async function getImagesFromFolder(folder: string = 'imageSD'): Promise<CloudinaryResource[]> {
  try {
    // Verificar configuración
    const config = cloudinary.config();
    if (!config.cloud_name || !config.api_key || !config.api_secret) {
      throw new Error('Cloudinary no está configurado correctamente. Verifique las variables de entorno.');
    }
...
export async function uploadImage(file: File, folder: string = 'imageSD'): Promise<CloudinaryResource | null> {
  try {
